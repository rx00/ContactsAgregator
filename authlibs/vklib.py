...
def auth(client_id, permissions, debug=False, debug_file="VK_Auth.log"):
    """
    ...
    debug_config(debug, debug_file)
    logging.debug("=== Auth Session ===")

    first_response = browser.open(request_builder(client_id, permissions))
    auth_answer = auth_form_parser(first_response.read().decode())
    join_response = browser.open(auth_answer)
    page_role = response_role(join_response)
    while True:
        if page_role[0] == 1:
            return token_parser(page_role[1])
        elif page_role[0] == 4:
            logging.debug("Bad login")
            ...
            page_role = response_role(response)
